...
- 새 테스트 파일: `labs/spring-lab/src/test/java/webboundary/HttpRequestResponseBoundaryTest.java`
- 테스트 메서드: `samePathWithDifferentMethodsAndBodies`
...
1. Java 객체가 HTTP를 통해 그대로 전달되지 않는 이유는 무엇인가?
2. 같은 경로인데 POST는 200, PUT은 405가 된 이유는 무엇인가?
3. 바이트코드와 HTTP 본문 바이트의 차이는 무엇인가?

## 면접 질문

1. “Controller가 반환한 객체가 클라이언트로 전달된다”라는 설명에서 생략된 변환 경계를 설명하라.
2. HTTP 요청의 메서드·경로·헤더·본문의 역할과 처리 코드 선택 기준을 설명하라.
...
- 검증일: 2026-07-30
- 실제 파일: `labs/spring-lab/src/test/java/webboundary/HttpRequestResponseBoundaryTest.java`
- 관련 테스트를 포함한 전체 테스트가 성공했다.
- 전체 테스트 결과: 22개 실행, 실패·오류 0개
- 최소 실험, assertion, 실패 분석, 학습자의 재설명과 완료 기준을 확인해 `WEB-01`을 완료로 확정했다.
