...
1. `ac0449c`에서 `labs/spring-lab` 실행 환경을 복구한다.
2. 생성자 주입 검증 코드를 실행해 두 구현체의 반환값을 확인한다.
3. 의도적으로 잘못된 기대값을 넣어 검증문이 실제로 실패하는지도 확인한다.
4. 다음 세션에서는 웹 진단 1번으로 진행한다.
