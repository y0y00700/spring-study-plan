...
실험 파일
새 테스트 파일:
labs/spring-lab/src/test/java/webboundary/ServletLifecycleContainerTest.java
테스트 메서드:
containerCreatesAndCallsServletLifecycle
프로덕션 코드는 수정하지 않았다.
WEB-01에서 추가한 다음 테스트 의존성을 그대로 사용했다.
...
LifecycleServlet은 다음 이벤트를 기록했다.
constructor
init
service
destroy
...
모든 테스트가 성공했다.
AssertJ 학습
@Test는 JUnit이 실행할 테스트 메서드를 지정한다.
assertThat은 AssertJ의 검증 시작 메서드다.
assertThat(actual).isEqualTo(expected) 형태로 실제값과 예상값을 비교한다.
containsExactly는 컬렉션의 개수, 내용, 순서를 모두 검증한다.
검증 실패 시 AssertionError가 발생하고 JUnit이 테스트를 실패로 처리한다.
완료 기준 재설명
...
없음
회상 문제
loadOnStartup=1인 Servlet에 요청이 3번 들어오면 각 생명주기 메서드는 몇 번 호출되는가?
ServletContextInitializer와 Servlet Container의 책임은 어떻게 다른가?
destroy 호출과 JVM 객체 메모리 회수는 왜 같은 사건이 아닌가?
면접 질문
Servlet의 전체 생명주기와 각 단계의 호출 주체를 설명하라.
Spring Boot 내장 Tomcat 환경에서 Spring ApplicationContext와 Servlet Container의 책임 경계를 설명하라.
다음 진행 제안
